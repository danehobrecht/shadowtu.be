...
from stem import Signal#!/usr/bin/env python
    ...
    while continuations:
        ...
        if action == 'action_get_comments':
            ...
            for continuation in section.get('continuations', []):
                ...
                continuations.append((ncd['continua#!/usr/bin/env python
# Test url: https://youtu.be/Y6ljFaKRTrI
        ...
        if isinstance(current_item, dict):
            for key, value in current_item.items():
                if key == search_key:
                    ...
                    stack.append(value)#!/usr/bin/env python
# Test url: https://youtu.be/Y6ljFaKRTrI
...
menuList()
menuInput()tion'], ncd['clickTrackingParams'], 'action_get_comments'))
            for item in section.get('contents', []):
                continuations.extend([(ncd['continuation'], ncd['clickTrackingParams'], 'action_get_comment_replies')
                                      for ncd in search_dict(item, 'nextContinuationData')])
...
def videosInput(): # Test url: https://youtu.be/Y6ljFaKRTrI
	global shareUrl
	shareUrl = raw#!/usr/bin/env python
# Test url: https://youtu.be/Y6ljFaKRTrI
...
menuList()
menuInput()_input("Enter YouTube share link: ")
	if 'https://youtu.be/' in shareUrl:
		try:
			checkTor = getTorSession().get("http://icanhazip.com").text
			videosExecute()
...
menuList()
menuInput()={'session_token': session_token},
                                headers={'X-YouTube-Client-Name': '1',
                                         'X-YouTube-Client-Version': '2.20201202.06.01'})
